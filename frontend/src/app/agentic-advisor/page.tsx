...
const initialActivity: ActivityItem[] = [
  {
    id: 1,
    title: "Detected rising disease risk — recommended monitoring",
    status: "Pending",
    timestamp: "Today, 10:42 AM · Field Sector B-12",
    body: "Satellite multispectral analysis indicates localized humidity anomalies in Sector B-12 consistent with early-stage fungal development. Our model suggests immediate deployment of ground sensors for verification.",
    actions: ["Review Details", "Dismiss"],
  },
  {
    id: 2,
    title: "Automated irrigation cycle complete — North Orchards",
    status: "Actioned",
    timestamp: "Today, 06:15 AM · North Field Complex",
    body: "Irrigation system optimized based on forecasted evapotranspiration rates. Water distribution was precisely focused on the Root Zone A areas.",
    stats: [
      { label: "Water Saved", value: "14,200 Liters", highlight: true },
      { label: "Soil Moisture", value: "68% Target Met" },
      { label: "Efficiency", value: "98.4%" },
    ],
  },
  {
    id: 3,
    title: "Harvest window prediction update — Corn Plot C",
    status: "Notified",
    timestamp: "Yesterday, 04:30 PM · Corn Plot C",
    body: "GDD (Growing Degree Days) accumulation suggests peak maturity will be reached between Oct 12-15. Recommendation: Logistical planning for harvesting equipment should begin within 48 hours.",
    faded: true,
  },
  {
    id: 4,
    title: "Soil nutrient deficiency alert — West Vineyard",
    status: "Actioned",
    timestamp: "Yesterday, 09:12 AM · West Vineyard",
    body: "Potassium levels detected below 120 ppm in Sector W-3. Automated fertigation mix adjusted. Variable rate application plan sent to connected machinery.",
  },
];
  ...
  return (
    <div className="mx-auto flex w-full max-w-[900px] flex-col gap-8 px-4 py-8 sm:px-8 sm:py-10">
      <div className="flex items-center justify-between">
        <h1 className="font-heading text-2xl font-bold text-slate-900 sm:text-3xl">Advisor activity</h1>
      ...
      <div className="flex flex-col">
            ...
            >
              ...
              {item.stats && (
                <div className="grid grid-cols-3 gap-4 rounded-lg border border-slate-100 bg-slate-50 p-4">
                  {item.stats.map((s) => (
                    <div key={s.label}>
                      <p className="text-[10px] font-bold uppercase tracking-wide text-text-faint">{s.label}</p>
        ...
        {activity.length === 0 && (
          <p className="py-10 text-center text-sm text-text-muted">No advisor activity right now.</p>
        )}
      </div>
