"use client";

import Link from "next/link";
import { Download, ShieldCheck, FileText, Trash2, ChevronRight } from "lucide-react";
import { useLanguage } from "@/lib/i18n/LanguageContext";
import type { TranslationKey } from "@/lib/i18n/LanguageContext";

const rows: { icon: typeof Download; title: TranslationKey; description: TranslationKey }[] = [
  {
    icon: Download,
    title: "dataPrivacy.download.title",
    description: "dataPrivacy.download.description",
  },
  {
    icon: ShieldCheck,
    title: "common.privacyPolicy",
    description: "dataPrivacy.privacyPolicy.description",
  },
  {
    icon: FileText,
    title: "login.footer.terms",
    description: "dataPrivacy.terms.description",
  },
];

export default function DataAndPrivacyPage() {
  const { t } = useLanguage();

  return (
    <div className="mx-auto flex w-full max-w-[800px] flex-col gap-8 px-4 py-8 sm:px-8 sm:py-10">
      <div>
        <h1 className="font-heading text-3xl font-bold text-slate-900">{t("dataPrivacy.title")}</h1>
        <p className="pt-1 text-base text-text-muted">{t("dataPrivacy.subtitle")}</p>
      </div>

      <div className="flex flex-col divide-y divide-slate-200 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        {rows.map((row) => {
          const Icon = row.icon;
          return (
            <button key={row.title} type="button" className="flex items-center justify-between gap-4 p-6 text-left">
              <div className="flex items-center gap-4">
                <div className="flex size-12 shrink-0 items-center justify-center rounded-xl bg-surface-muted">
                  <Icon className="size-5 text-slate-700" />
                </div>
                <div>
                  <p className="font-bold text-slate-900">{t(row.title)}</p>
                  <p className="text-sm text-text-muted">{t(row.description)}</p>
                </div>
              </div>
              <ChevronRight className="size-3.5 shrink-0 text-slate-400" />
            </button>
          );
        })}
        <button type="button" className="flex items-center justify-between gap-4 p-6 text-left">
          <div className="flex items-center gap-4">
            <div className="flex size-12 shrink-0 items-center justify-center rounded-xl bg-red-50">
              <Trash2 className="size-5 text-red-600" />
            </div>
            <div>
              <p className="font-bold text-red-600">{t("dataPrivacy.deleteAccount")}</p>
              <p className="text-sm text-text-muted">{t("dataPrivacy.deleteAccountBody")}</p>
            </div>
          </div>
          <ChevronRight className="size-3.5 shrink-0 text-slate-400" />
        </button>
      </div>

      <div className="flex items-center justify-center gap-4">
        <Link href="/settings" className="rounded-xl border border-slate-200 bg-white px-6 py-3 text-sm font-bold text-slate-700 shadow-sm">
          {t("helpCenter.backToSettings")}
        </Link>
        <button type="button" className="rounded-xl bg-brand px-6 py-3 text-sm font-bold text-white shadow-lg">
          {t("dataPrivacy.savePreferences")}
        </button>
      </div>
    </div>
  );
}
